...
modules.forEach((config) => {
  ...
  if (config.name == "transform") {
    ...
    let list = ["transform.js"].concat(
      fs.readdirSync(dir).filter((n) => /\.js$/.test(n) && n != "transform.js")
    );
...
function getImport(name) {
  ...
  if (!found) {
    ...
    for (let prop in read[name].all) found[prop] = "#" + name + "." + prop;
  }
  return found;
}
...
let toc = [{ name: "Intro", href: "#top.intro" }],
  output = modules.map((module) => {
    let tocPart = {
      name: "prosemirror-" + module.name,
      href: "#" + module.name,
    };
    toc.push(tocPart);
    let text = builddocs
      ...
      .replace(/<h3>(.*?)<\/h3>/g, function (_, text) {
