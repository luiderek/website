...
module.exports = function loadTemplates(config) {
  ...
  fs.readdirSync(config.dir).forEach(function (filename) {
    ...
    if (match)
      mold.bake(
        match[1],
        fs.readFileSync(config.dir + match[1] + ".html", "utf8").trim()
      );
  });
