...
class EditorConnection {
        ...
        if (action.type == "loaded") {
            ...
            let editState = EditorState.create({
                ...
                plugins: exampleSetup({
                    ...
                    commentPlugin,
                    commentUI(transaction => this.dispatch({
                        type: "transaction",
                        transaction
                    }))
                ]),
        ...
        if (this.state.edit) {
            if (this.view)
                ...
                this.setView(new EditorView(document.querySelector("#editor"), {
                    ...
                    dispatchTransaction: transaction => this.dispatch({
                        type: "transaction",
                        transaction
                    })
                }))
    ...
    sendable(editState) {
        ...
        if (steps || comments.length) return {
            steps,
            comments
        }
    }
    ...
    recover(err) {
        ...
        setTimeout(() => {
            if (this.state.comm == "recover") this.dispatch({
                type: "poll"
            })
        }, this.backOff)
    }
...
// (1 user) or (23 users). Cute tertiary function.
...
function showDocList(node, list) {
    ...
    let ul = docList = document.body.appendChild(crel("ul", {
        class: "doclist"
    }))
    list.forEach(doc => {
    ...
    ul.addEventListener("click", e => {
        if (e.target.nodeName == "LI") {
            ...
            if (e.target.hasAttribute("data-name"))
                ...
                newDocument(prompt("Name the new document:", ""))
        }
    })
}

// doclist is the UL element containing the li's with the data-name attribute.
// this simply closes the document selection UI when clicked on.
...
// How to create new documents. This should be an important line for me in the future.
// I need to figure out how to call this when specific conditions are met.
...
function connectFromHash() {
    // Regex for '#edit-reallywhatevergoeshere'
    // returns an array of locations where the regex was found in a string
